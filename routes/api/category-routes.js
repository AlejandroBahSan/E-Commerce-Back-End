  ...
  try {
    ...
    return dbCategory
      ...
      : res
          .status(404)
          .json({
            message: `No Category found with the following id ${dbCategory}`,
          });
  } catch (err) {
    ...
    .then((dbCategory) => {
      ...
      if (!dbCategory[0]) {
        res
          .status(404)
          .json({
            message: `No category found with id ${req.params.id} or maybe you are trying to assign the same name to this row`,
          });
        return;
      }
      res.status(202).json({
  ...
  Category.destroy({
    ...
    .then((dbCategory) => {
      return dbCategory
        ? res
            .status(200)
            .json({ message: `Category Id ${req.params.id} destroyed` })
        : res
            .status(404)
            .json({
              message: `No Category found with the following id ${req.params.id}`,
            });
    })
    .catch((err) => {
      console.log(err);
