...
router.get('/:id', async (req, res) => {
  try {
    ...
    return dbTag ? res.status(200).json(dbTag) : res.status(404).json({
...
router.put('/:id', async (req, res) => {
  Tag.update(
    ...
    .then(dbTag => {
      ...
      if (!dbTag[0]) {
        res.status(404).json({ message: `No Tag found with id ${req.params.id} or you are trying to assign the same tag name to this id` });
        return;
      }
      res.status(202).json({
...
router.delete('/:id', (req, res) => {
  Tag.destroy({
    ...
    .then(dbTag => {
      ...
      : res.status(500).json({ message: `Sorry no Category found with the following id ${req.params.id}` });  
  })
    .catch(err => {
        console.log(err);
        res.status(500).json(err);
